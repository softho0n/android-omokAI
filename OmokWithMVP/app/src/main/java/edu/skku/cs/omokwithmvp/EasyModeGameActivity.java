...
public class EasyModeGameActivity extends AppCompatActivity {
    ...
    public void updateScoreinDatabase() {
        ...
        HttpUrl.Builder urlBuilder =
                HttpUrl.parse("https://f3dzusuokg.execute-api.ap-northeast-2.amazonaws.com/dev/update/easyscore").newBuilder();
    ...
    public void onBackPressed() {
        Intent intent = new Intent(EasyModeGameActivity.this, MenuActivity.class);
        intent.putExtra("name", name);
        intent.putExtra("hardscore", hardscore);
        if(Integer.parseInt(first_easyscore) < scoreCopy) {
            intent.putExtra("easyscore", String.valueOf(scoreCopy));
        }
        else {
            intent.putExtra("easyscore", first_easyscore);
        }
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                if(sharding.get(i).getIsEmpty() == true) {
                    ...
                    if(checkVictory(1)) {
                        ...
                        new NordanAlertDialog.Builder(EasyModeGameActivity.this)
                                .isCancellable(false)
                                ...
                                .onPositiveClicked(() -> {
                                    ...
                                    intent.putExtra("name", name);
                                    intent.putExtra("hardscore", hardscore);
                                    if(Integer.parseInt(first_easyscore) < userScore) {
                                        intent.putExtra("easyscore", String.valueOf(userScore));
                                    }
                                    else {
                                        intent.putExtra("easyscore", first_easyscore);
                                    }
                    ...
                    urlBuilder.addQueryParameter("type", "easy");
                    passStr = new String();
                    ...
                    client.newCall(req).enqueue(new Callback() {
                        ...
                        public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                            ...
                            EasyModeGameActivity.this.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    ...
                                    if(checkVictory(-1)) {
                                        ...
                                        for(int k=0; k<5; k++) {
                                            //System.out.println(victory_pos.get(k).getY());
